...
export async function startSolicitation({ order, customer, ipnUrl, returnUrl }) {
  ...
  const body = {
    order: {
      ...
      merchant: {
  id: process.env.SEQURA_MERCHANT,
  approved_url: returnUrl,
  cancelled_url: returnUrl + '&status=cancelled',
  rejected_url: returnUrl + '&status=rejected',
},
      cart: {
        currency: 'EUR',
        gift: false,
  ...
  if (!location) throw new Error('Sequra no devolvio Location');
  const orderUrl = location.startsWith('http') ? location : `${BASE_URL}${location}`;
  console.log('Sequra orderUrl:', orderUrl);
  return { orderUrl };
}

export async function fetchForm(orderUrl, cuotasMax, approvedUrl) {
